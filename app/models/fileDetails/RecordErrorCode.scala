...
import play.api.libs.json.{__, JsString, JsValue, Reads, Writes}
...
object RecordErrorCode {
  ...
  given Writes[RecordErrorCode] with
    def writes(x: RecordErrorCode): JsValue = JsString(x.code)

  given Reads[RecordErrorCode] =
    __.read[String].map {
