        ...
        case Some(uploadId) =>
          upscanConnector.getUploadStatus(uploadId) flatMap {
            case Some(_: UploadedSuccessfully) =>
              Future.successful(Ok(Json.toJson(RedirectAsJson(routes.FileValidationController.onPageLoad().url))))
            case Some(r: UploadRejected) =>
              if (r.details.message.contains("octet-stream")) {
                ...
                Future.successful(Ok(Json.toJson(RedirectAsJson(routes.UploadFileController.showError("OctetStream", errorReason, "").url))))
              } else {
                logger.debug(s"Upload rejected. Error details: ${r.details}")
                Future.successful(Ok(Json.toJson(RedirectAsJson(routes.NotXMLFileController.onPageLoad().url))))
              }
            case Some(Quarantined) =>
              Future.successful(Ok(Json.toJson(RedirectAsJson(routes.VirusFileFoundController.onPageLoad().url))))
            case Some(Failed) =>
