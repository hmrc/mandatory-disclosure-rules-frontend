...
class UploadFileControllerSpec extends SpecBase with ScalaCheckPropertyChecks with Generators {
  ...
  "upload file controller" - {
    ...
    "must read the progress of the upload from the backend" in {
      ...
      def verifyResult(uploadStatus: UploadStatus, expectedStatus: Int = OK, expectedResult: Option[RedirectAsJson] = None): Unit = {
        ...
        val actualResult = Option(contentAsString(result)).collect { case x if x.trim.nonEmpty => x }.map(Json.parse(_).as[RedirectAsJson])
        actualResult mustBe expectedResult
        application.stop()
      }

      verifyResult(InProgress, CONTINUE, None)
      verifyResult(Quarantined, OK, Some(RedirectAsJson("/report-under-mandatory-disclosure-rules/report/problem/virus-file-found")))
      verifyResult(
        UploadRejected(ErrorDetails("REJECTED", "message")),
        OK,
        Some(RedirectAsJson("/report-under-mandatory-disclosure-rules/report/problem/not-xml-file"))
      )
      verifyResult(Failed, OK, Some(RedirectAsJson("/report-under-mandatory-disclosure-rules/report/problem/there-is-a-problem")))
      verifyResult(UploadedSuccessfully("name", "downloadUrl"), OK, Some(RedirectAsJson("/report-under-mandatory-disclosure-rules/report/file-validation")))
