...
class FileStatusControllerSpec extends SpecBase {
  ...
  "FileStatus Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val fileDetails = Seq(FileDetails("name.xml", LocalDateTime.now(), LocalDateTime.now(), Pending, "id"))
      when(mockFileConnector.getAllFileDetails(any())(any(), any())).thenReturn(Future.successful(Some(fileDetails)))
    ...
    "must return INTERNAL_SERVER_ERROR on failing to get allFileDetails" in {
      ...
      when(mockFileConnector.getAllFileDetails(any())(any(), any())).thenReturn(Future.successful(None))
